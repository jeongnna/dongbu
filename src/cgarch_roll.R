...
cgarch_auto <- function(data, set.length, dist.model, var.model, time.v, copula){
  ...
  multi <- replicate(n=ncol(data), expr=ugarch) %>% 
    multispec()
  ...
  value <- list(MGARCH=c.fitted, UGARCH=u.fitted)
  return(value)
}
...
cgarch_vcov <- function(data, dist.model='std', var.model='eGARCH', time.v=FALSE, copula='mvt') {
  data <- as.data.frame(data)
...
margins<-function(fitted, dist=NULL){
  normalized.res<-fitted@fit$residuals / fitted@fit$sigma
  if (is.null(dist)){
    if (is.na(coef(fitted)['skew'])){
      margins<-pdist(distribution='std', q=normalized.res,
                     shape=coef(fitted)['shape'])
    } else {
      margins<-pdist(distribution='sstd', q=normalized.res,
  ...
  if (dist=='norm'){
    margins<-pdist(distribution=dist, q=normalized.res, mu=0, sigma=1)
  }
  return(margins)
}
